...
def crop_image_and_create_thumbnail(event, context):
    ...
    if 'Side1' in args["image_key"]:
        upload_key = cropImage.get_thumbnail_name(args["image_key"])
        thumbnail_output = cropImage.crop_and_upload_thumbnail(source_key=args["image_key"],upload_key=upload_key)
        output = output + thumbnail_output
