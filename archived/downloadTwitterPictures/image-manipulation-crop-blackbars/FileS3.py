...
class FileS3(FileOs):
    ...
    def read_image(self, key):
        image_data = self.read_file(key)
        img = Image.open(BytesIO(image_data))
        return img
        
    def upload_image_to_s3(self, img, key):
        image_bytes = self.convert_img_to_bytes(img, 'JPEG')
        self.put_file(key, image_bytes)

    def convert_img_to_bytes(self, img, format):
        in_mem_file = BytesIO()
